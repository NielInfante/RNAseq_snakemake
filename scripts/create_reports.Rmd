...
tableToShow <- res %>% select(GeneName, baseMean, log2FoldChange, padj, Biotype)
...
datatable(tableToShow, filter='bottom', options = list(pageLength=10)) %>% 
...
Genes are filtered if the average expression across all samples is less than the cut off value, in this case `r round(2^cut_value)`.

## Significant Genes
A total of `r dim(sig)[1]` genes were counted as significant in this test. Genes are counted as significant if their base mean is above `r signig(2^cut_value, digits=2)`, the adjusted p value is below 0.05, and there is a fold change of at least 1.5.
...
datatable(tableToShow, filter='bottom', options = list(pageLength=10, scrollX=T)) %>% 
	formatRound('log2FoldChange', 3) %>% 
	formatRound('baseMean', 0)
